...
class Shamsi
{
    ...
    public function jNow($time)
    {
        $list = explode(' ', $this->jdate( date('Y-m-d-D H:i:s', $time), true, false ) );
        return
            $list[3].'/'.$list[2].'/'.$list[1]." ". date("H:i", $time);
    ...
    public function jdate($datetime, $hastime=false, $localize=true)
    {
        ...
        $d = $this->gregorian_to_jalali_($date);
        return $d . " " . $time;
    ...
    function jalali_to_gregorian($m)
    {
        $g_days_in_month = array(31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31);
        $j_days_in_month = array(31, 31, 31, 31, 31, 31, 30, 30, 30, 30, 30, 29);
    ...
    function gregorian_to_jalali($m)
    {
        try{
            $g_y = explode("/",$m)[0];
            $g_m= explode("/",$m)[1];
    ...
    function gregorian_to_jalali_($m)
    {
        $g_y = explode("/",$m)[0];
        $g_m= explode("/",$m)[1];
        $g_d = explode("/",$m)[2];
