                        ...
                        <div class="row">
                            ...
                            @if (auth()->user()->hasRole('admin'))
                                @if ($user->hasRole('supervisor'))
                                    <div class="form-group col-lg-3">
                                        ...
                                        <label>{{ __('Max workers') }}</label>

                                        <input type="text" value="{{ old('max_workers', $user->max_workers) }}"
                                            name="max_workers"
                                            class="form-control @error('max_workers') is-invalid @enderror">
                                    </div>
                                @endif
                                <div class="form-group col-lg-4">
                                    ...
                                    <select name="roles[]" multiple class="form-control select2">
                                        @foreach (\Spatie\Permission\Models\Role::all() as $role)
                                            <option @if ($user->hasRole($role->id)) selected @endif
                                                value="{{ $role->id }}">
                                                {{ $role->name }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group col-lg-4">
                                    ...
                                    <select name="supervisor" @if ($user->hasRole(['admin', 'supervisor'])) disabled @endif
                                        class="form-control ">
                                        <option>
                                            Select ...</option>
                                        @foreach ($supervisors as $supervisor)
                                            <option @if (($user->supervisor()->first()?->id ?? 0) == $supervisor->id) selected @endif
                                                value="{{ $supervisor->id }}">
                            ...
                            <div class="form-group col-lg-3">
                                ...
                                <div class="form-check">
                                    <input type="checkbox" name="remotable" class="form-check-input" value="1"
                                        id="exampleCheck2" @if (old('enabled', $user->remotable)) checked @endif>
                            ...
                            <div class="form-group col-lg-3">
                                ...
                                <input type="password" value="{{ old('password_confirm') }}" name="password_confirm"
