                ...
                <form action="{{ route('admin.settings.store') }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="form-group col-lg-4">
                                <label>api</label>
                                <input type="hidden" name="settings[modal_info][api]" value="{{ $items['modal_info']['api'] ?? '' }}">
                                <div class="custom-file mb-2">
                                    <input type="file" name="settings[modal_info][api]" onchange="readURL(this)" class="custom-file-input" id="customFile1">
                                    <label class="custom-file-label" for="customFile1">{{ $items['modal_info']['api'] ?? 'Choose file' }}</label>
                                </div>
                                <img class="img-fluid img-rounded pic-preview bg-light object-fit-cover" @if(!empty($items['modal_info']['api'])) src="{{ asset($items['modal_info']['api']) }}" @endif width="150" height="150">
                            </div>
                        </div>
                    </div>
                    <!-- /.card-body -->
